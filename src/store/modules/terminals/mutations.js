...
export default {
  ...
  getTerminals(state, terminalList) {
    state.terminals = terminalList.data.terminals;
    state.terminalsTotal = terminalList.data.total;
  },
  //添加终端列表信息
  ...
  getTerminalById: (state, id) =>
    state.terminals.forEach(terminal => terminal.id === Number(id) && (state.terminalForEdit = terminal)),
  //编辑终端信息

<template>
  <el-row id="complaints-proposal" :gutter="0">
    <el-row :gutter="0" v-if="!isComplaintsProposal">
      <!--添加狱务公开信息-->
      <el-col :span="24">
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      $route(to, from) {
        if (to.path === '/news/complaints_proposals') {
          this.isComplaintsProposal = false;
          this.breadCrumb(this.breadcrumb);//路由发生改变重新发送面包屑信息
        } else {
          this.isComplaintsProposal = true;
        }
      }
    },
    computed: {
